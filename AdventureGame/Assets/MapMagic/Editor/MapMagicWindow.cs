...
namespace MapMagic
{
	public partial class MapMagicWindow : EditorWindow, ISerializationCallbackReceiver
	{
		...
		public delegate Item[] DrawPopupDelegate (Item[] items, Vector2 mousePos, GeneratorsAsset graph, Generator clickedGen, Group clickedGroup, Generator.Output clickedOutput);
		...
		public void DrawPopup ()
		{
			...
			foreach (System.Type type in typeof(Generator).Subtypes())
			{
				if (System.Attribute.IsDefined(type, typeof(GeneratorMenuAttribute)))
				{
					...
					Item item = new Item(attribute.name, onClick:() => CreateGenerator(genType, mousePos) );
					item.priority = attribute.priority;

					if (attribute.menu.Length != 0)
					{
						if (!itemsDict.ContainsKey(attribute.menu)) itemsDict.Add(attribute.menu, new Item(attribute.menu, new Item[0]));
						itemsDict[attribute.menu].subItems.Add(item);
					}
			...
			Item[] createItems = new Item[itemsDict.Count];
			itemsDict.Values.CopyTo(createItems, 0);

			//create group
			//Item createGroupItem = new Item("Group",  delegate () { CreateGroup(mousePos); });
			...
			//preview
			Item[] previewSubs = new Item[]
			{
				new Item("On Terrain", onClick:()=> {PreviewOutput(clickedGenerator, clickedOutput, false);}, disabled:clickedOutput==null||clickedGenerator==null, priority:0), 
				new Item("In Window", onClick:()=> {PreviewOutput(clickedGenerator, clickedOutput, true);}, disabled:clickedOutput==null||clickedGenerator==null, priority:1),
				new Item("Clear", onClick:()=> {PreviewOutput(null, null, false);}, priority:2 )//, disabled:MapMagic.instance.previewOutput==null)
			};

			Item[] popupItems = new Item[]
			{
				new Item("Create", createItems) {priority=0},
				new Item("Export",	onClick:()=> { ExportGenerator(clickedGenerator, mousePos); }, priority:10),
				new Item("Import",	onClick:()=> { ImportGenerator(mousePos); }, priority:20),
				new Item("Duplicate",	onClick:()=> { DuplicateGenerator(clickedGenerator); }, priority:30),
				new Item("Update",	onClick:()=> { UpdateGenerator(clickedGenerator); }, disabled:clickedGenerator==null || !CouldBeUpdated(clickedGenerator), priority:40),
				new Item("Remove",	onClick:()=> { if (clickedGenerator!=null) DeleteGenerator(clickedGenerator); },	disabled:clickedGenerator==null, priority:50),
				new Item("Reset",	onClick:()=> { if (clickedGenerator!=null) ResetGenerator(clickedGenerator); },	disabled:clickedGenerator==null, priority:60), 
				new Item("Preview", previewSubs) {priority=70}
			};
			...
			Item root = new Item("Menu");
			root.subItems = new List<Item>();
			root.subItems.AddRange(popupItems);

			SingleWindow menu = new SingleWindow(root);
