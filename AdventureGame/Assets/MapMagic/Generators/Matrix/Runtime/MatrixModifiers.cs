...
//		#endif
	...
	public class Erosion200 : Generator, IInlet<MatrixWorld>, IOutlet<MatrixWorld>, ICustomComplexity
	{
		...
		public override void Generate (TileData data, StopToken stop)
		{
			...
			MatrixWorld dst = new MatrixWorld(src);
			Erosion(dst, data.isDraft, data, iterations, terrainDurability, erosionAmount, sedimentAmount, fluidityIterations, this, stop);
				if (stop!=null && stop.stop) return;
		...
		public static void Erosion (MatrixWorld dstHeight, bool isDraft, TileData data, 
			int iterations, float terrainDurability, float erosionAmount, float sedimentAmount, int fluidityIterations,
