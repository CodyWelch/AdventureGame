...
{
	[CustomPropertyDrawer(typeof(DecisionScoreEvaluatorPickerAttribute), true)]
	public class DecisionScoreEvaluatorPickerDrawer : PropertyDrawer
	{
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			EditorGUI.BeginProperty(position, label, property);
			DecisionScoreEvaluator current = property.GetValue() as DecisionScoreEvaluator;
			position = EditorGUI.PrefixLabel(position, label);
			DoSelection(position, property, label, current);
			EditorGUI.EndProperty();
		}

		protected virtual void DoSelection(Rect buttonRect, SerializedProperty property, GUIContent label, DecisionScoreEvaluator current)
		{
		...
		protected virtual List<DecisionScoreEvaluator> GetItems(UtilityDatabase database)
		{
			...
			return database.decisionScoreEvaluators.Where(x => type.IsAssignableFrom(x.GetType())).ToList();
		}
